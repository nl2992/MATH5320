...
from src.config import DEFAULT_BACKTEST_MODEL
...
st.caption(
    "Historical · Parametric (Delta-Normal) · Monte Carlo VaR/ES  |  "
    "Stocks & European Options  |  Kupiec Backtesting"
)

# ── Tabs ───────────────────────────────────────────────────────────────────────
tab_portfolio, tab_data, tab_settings, tab_results, tab_backtest = st.tabs(
    [
        "1 · Portfolio Input",
        "2 · Market Data",
        "3 · Risk Settings",
...
with tab_results:
    ...
    if not prices_ready:
        ...
        current_params: dict = st.session_state.get("risk_params", risk_params)
        ...
        if ticker_errors:
            ...
            if "results" in st.session_state:
                render_results_panel(
                    ...
                    lookback_days=st.session_state.get("risk_params", risk_params)[
                        "lookback_days"
                    ],
                    var_confidence=st.session_state.get("risk_params", risk_params)[
                        "var_confidence"
                    ],
                )
...
with tab_backtest:
    ...
    if st.session_state.get("prices") is None:
        ...
        current_params = st.session_state.get("risk_params", risk_params)
...
def _render_backtest_results(bt_result: dict, params: dict) -> None:
    ...
    if bt_df.empty:
        st.warning("Not enough data for backtesting. Try a shorter lookback window.")
        return
