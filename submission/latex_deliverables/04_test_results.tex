...
No-network tests & \textbf{644 passed, 0 failed, 1 intentionally skipped} \\
...
\small\itshape
Observed test outcomes for the MATH5320 risk engine: unit suite,
integration scripts, analytical benchmarks, homework fixtures,
walk-forward backtesting, and coverage.
\end{minipage}
\end{titlepage}
...
644 passed, 1 intentional skip, 0 failures.  95\% statement coverage.
Both integration scripts passed.

\textbf{No-network suite:} 644 tests, 1 skip.  Untested lines are in UI
branch paths, selected credit-service helpers, and defensive validation
guards.

\textbf{Integration:} Both live-data scripts passed end to end — data
download, service orchestration, risk-model execution, backtesting.

\textbf{Conclusion:} The deterministic tests cover all core formulas;
integration confirms the full pipeline.  The implementation is correct
...
{\footnotesize \code{streamlit 1.37.1}, \code{numpy 1.26.4},
\code{pandas 3.0.2}, \code{scipy 1.17.1},
\code{plotly 5.24.1}, \code{yfinance 1.2.0},
...
\begin{itemize}
  ...
  \item \code{integration\_test\_output.txt}, \code{integration\_test\_\allowbreak formula\_sheet\_output.txt}
  \item \code{per\_file\_\allowbreak test\_counts.json},
    \code{homework\_fixture\_\allowbreak results.csv}
  \item \code{official\_benchmark\_\allowbreak results.csv},
    \code{backtest\_results.csv}
\end{itemize}
...
\begin{center}
\footnotesize
\begin{tabular}{L{2.4cm}L{4.5cm}rrrL{3.5cm}}
\toprule
\textbf{Group} & \textbf{File(s)} & \textbf{Passed} & \textbf{Failed} & \textbf{Skipped} & \textbf{Notes} \\
\midrule
Core backend              & \code{test\_backend.py}                                   & 29  & 0 & 0 & Core pricing, portfolio, VaR/ES, service smoke \\
Backtest extensions       & \code{test\_backtest\_\allowbreak extensions.py}          & 31  & 0 & 0 & Christoffersen, conditional coverage, traffic light \\
Course validation         & \code{test\_course\_\allowbreak validation.py}            & 67  & 0 & 0 & Course formula-sheet and regression fixtures \\
Homework fixtures         & \code{test\_homework\_\allowbreak cases.py}               & 83  & 0 & 0 & Homework-derived validation cases \\
Lognormal                 & \code{test\_lognormal.py}                                 & 34  & 0 & 0 & Exact GBM/lognormal formulas \\
Credit                    & \code{test\_credit.py}, \code{test\_cva\_\allowbreak mitigants.py}, \code{test\_counterparty\_\allowbreak mitigation.py}, \code{test\_merton\_\allowbreak timing.py} & 118 & 0 & 0 & Hazard, Merton, CDS, CVA, mitigants \\
Credit service            & \code{test\_credit\_\allowbreak service.py}               & 11  & 0 & 0 & Extension-service aggregation \\
Regulatory                & \code{test\_regulatory.py}, \code{test\_balance\_\allowbreak sheet.py}, \code{test\_dfast\_\allowbreak pathing.py} & 46 & 0 & 0 & RWA, capital ratio, stress pathing \\
Market data               & \code{test\_market\_\allowbreak data.py}                  & 25  & 0 & 0 & CSV loader, yfinance wrappers, cache, rate helper \\
Config / validation       & \code{test\_config\_and\_\allowbreak validation.py}, \code{test\_packaging\_\allowbreak namespace.py} & 22 & 0 & 0 & Input validation and package namespace \\
Charts                    & \code{test\_charts.py}                                    &  6  & 0 & 0 & Plot helpers \\
UI panels                 & \code{test\_ui\_panels.py}                                & 68  & 0 & 0 & Streamlit panel behavior \\
Coverage and numerics     & \code{test\_coverage\_\allowbreak gaps.py}, \code{test\_strict\_\allowbreak numerics.py}, \code{test\_es\_confidence\_\allowbreak split.py} & 84 & 0 & 0 & Gap-closing and numerical-discipline tests \\
Integration               & \code{integration\_test.py}                               &  1  & 0 & 0 & Live-data end-to-end market-risk workflow \\
Formula integration       & \code{integration\_test\_\allowbreak formula\_sheet.py}   &  1  & 0 & 0 & Live-data formula-sheet workflow \\
\midrule
\textbf{Total}            & &  \textbf{644} & \textbf{0} & \textbf{0} & \\
\bottomrule
\end{tabular}
\end{center}
...
No-network tests skipped   & 1 (intentional) \\
...
\begin{center}
\footnotesize
\begin{tabular}{L{1.5cm}L{2.1cm}L{3.7cm}L{3.7cm}L{2cm}l}
\toprule
...
\begin{sloppypar}
The file \code{submission/\allowbreak test\_artifacts/\allowbreak homework\_fixture\_\allowbreak results.csv}
...
\begin{sloppypar}
The file \code{submission/\allowbreak test\_artifacts/\allowbreak official\_\allowbreak benchmark\_results.csv}
...
The test suite of 644 cases varies widely in informational density.
...
\begin{sloppypar}
Data validation is covered by \code{tests/\allowbreak test\_market\_data.py}
(24 passing tests) and \code{tests/\allowbreak test\_config\_and\_\allowbreak validation.py}
(10 passing tests).
\end{sloppypar}
...
\begin{center}
\footnotesize
\begin{tabular}{L{4.2cm}rL{3.8cm}L{4.2cm}}
\toprule
\textbf{File} & \textbf{Missing} & \textbf{Likely reason} & \textbf{Fix direction} \\
\midrule
...
\code{src/ui/\allowbreak capital\_panel.py}& 18 & UI branches not fully exercised & Add panel-path tests \\
\code{src/ui/\allowbreak cds\_cva\_panel.py}& 18 & UI and branch-heavy error paths & Add panel-path tests \\
\code{src/risk/historical.py}             & 16 & Historical vol-shock paths & Add explicit historical-shock tests \\
\code{src/services/\allowbreak regulatory\_\allowbreak service.py}& 11 & Capital-path helper branches & Add service-branch tests \\
...
\code{src/risk/\allowbreak estimators.py}  &  7 & Manual-parameter validation & Add targeted manual-input tests \\
\code{src/credit/\allowbreak mitigation.py}&  4 & Less common mitigation branches & Add more mitigant tests \\
\code{src/risk/returns.py}                &  3 & Absolute-return helper path & Add branch tests \\
\code{src/risk/\allowbreak regulatory.py}  &  2 & Small residual branches & Add targeted tests \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{L{5cm}L{3.5cm}L{2.8cm}L{3.2cm}}
\toprule
...
\code{tests/\allowbreak integration\_test.py}          & Passed & No & Live workflow evidence \\
\code{tests/\allowbreak integration\_test\_\allowbreak formula\_sheet.py} & Passed & No & Live formula-sheet evidence \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item Deterministic formula modules pass all 644 no-network tests.
...
\begin{itemize}
  ...
  \item \code{integration\_test\_\allowbreak formula\_sheet\_output.txt}: terminal output of formula-sheet integration test
  ...
  \item \code{homework\_fixture\_\allowbreak results.csv}: homework regression fixture results
  \item \code{official\_benchmark\_\allowbreak results.csv}: external benchmark comparison results
\end{itemize}
