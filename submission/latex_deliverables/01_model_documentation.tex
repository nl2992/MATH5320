...
\textbf{Validation methodology.}  Validation was performed through a 644-test
...
nonlinear option books.  The primary limitations include but are not limited to, option repricing uses fixed implied volatility or a simplified
vol-shock approximation rather than a full implied-volatility surface (for practical reasons) the
...
\end{equation}
Hence, when the user supplies an effective window length $N$, the course
implementation uses
\begin{equation}
  N = \frac{1}{1-\lambda},
  \qquad
...
design; the 644-test no-network unit suite with 95\% statement coverage; two
