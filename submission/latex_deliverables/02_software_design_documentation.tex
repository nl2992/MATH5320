...
\code{tests/} provides comprehensive validation coverage.
...
to be read and understood in isolation.  Extension modules — credit,
CVA, regulatory — can be added without modifying the application shell
or the core stock-and-option risk path.
...
\begin{center}
\begin{tabular}{llL{3.5cm}L{4.5cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Rule} & \textbf{Validation behaviour} \\
\midrule
...
\textbf{610 passed, 0 failed, 0 skipped}.
...
Recorded result: \textbf{96\% statement coverage}.  Remaining untested
...
96\% statement coverage rather than 100\% & Some UI branches and defensive paths are untested & Remaining gaps are non-formula paths; documented in Section~10.3 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{Testing.}  Section~10 documents the no-network test suite
    (610 tests, 96\% coverage), integration scripts, and coverage
    remediation plan.
