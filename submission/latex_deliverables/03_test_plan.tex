...
\small\itshape
The validation program for the MATH5320 risk engine: what is tested,
why, against what benchmark, and what passes.
\end{minipage}
\end{titlepage}
...
Four questions drive this plan: what is tested, why, against what
benchmark, and what constitutes a pass.

The test suite covers formula correctness, portfolio valuation,
VaR/ES, backtesting, market-data loading, UI behaviour, and
course-formula extensions.  We treat tests as validation evidence,
not just execution checks.  Section~11 adds explicit benchmark
comparisons against analytical references.
...
covariance estimation, historical/parametric/Monte Carlo VaR and ES,
VaR backtesting and exception diagnostics, course-formula extensions
(\code{lognormal.py}, \code{src/credit/}, \code{regulatory.py}),
market-data loading and validation, and Streamlit UI rendering.
...
\code{tests/test\_numerical\_precision.py}& 7 tests covering IEEE 754 numerical precision and failure modes \\
\bottomrule
\end{tabular}
\end{center}
...
This comparison is implemented in \code{test\_backend.py} and
\code{test\_course\_validation.py}; results are recorded in
\code{test\_artifacts/official\_benchmark\_results.csv}.

\subsection{Monte Carlo vs.\ Exact GBM Benchmark}

For a single-asset position, Monte Carlo VaR should converge to the
exact closed-form lognormal VaR as the simulation count grows.
We verify:

\begin{itemize}
  \item At $n_{\mathrm{sims}} = 100{,}000$ (fixed seed), Monte Carlo 99\%
    VaR agrees with \code{var\_long\_lognormal} within 2\% for a
    single-stock case.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Numerical Precision and Failure-Mode Testing}
\label{sec:numerical-precision-failure-mode}

Quantitative developers need to understand both the strengths and the
weaknesses of the numerical methods they use.  The test suite therefore
includes a dedicated numerical-precision file,
\code{tests/test\_numerical\_precision.py}, designed to address floating-point
failure modes discussed in the lectures.  The relevant numerical reference
is Goldberg's discussion of IEEE 754 floating-point arithmetic \cite{Gol91}.

These tests are not intended to prove that the model is accurate for every
market regime.  Their purpose is narrower and more technical: they confirm
that the implementation remains finite, stable, and directionally sensible
under cases that commonly expose floating-point errors, catastrophic
cancellation, unstable covariance calculations, and extreme-tail VaR/ES
evaluation.

\begin{longtable}{L{2.2cm}L{4.4cm}L{6.2cm}}
\toprule
\textbf{Test ID} & \textbf{Failure mode} & \textbf{Validation purpose} \\
\midrule
\endfirsthead
\toprule
\textbf{Test ID} & \textbf{Failure mode} & \textbf{Validation purpose} \\
...
\bottomrule
\endfoot
\code{NP\_01} & Black-Scholes underflow at very low volatility & Confirms that an at-the-money call with $\sigma = 10^{-10}$ returns a finite value close to discounted intrinsic value rather than NaN or infinity. \\
\code{NP\_02} & Black-Scholes extreme high volatility & Confirms that a call with $\sigma = 50$ remains finite, positive, and bounded by the underlying spot scale. \\
\code{NP\_03} & Near-zero time to maturity & Confirms that a deep in-the-money call with $T = 10^{-8}$ prices close to intrinsic value and does not fail from division by a very small denominator. \\
\code{NP\_04} & Catastrophic cancellation in log returns & Confirms that very small price increments produce finite nonzero log returns of the correct order of magnitude. \\
\code{NP\_05} & Near-singular covariance in VaR & Confirms that a highly correlated, nearly singular covariance setting does not crash the parametric VaR calculation and returns finite positive VaR. \\
\code{NP\_06} & EWMA long-series stability & Confirms that EWMA mean and covariance over a 2000-day series remain finite, symmetric, and positive semidefinite up to numerical tolerance. \\
\code{NP\_07} & Extreme-confidence VaR/ES & Confirms that parametric VaR and ES at $\alpha = 0.9999$ remain finite, positive, and satisfy ES $\geq$ VaR at the same confidence level. \\
\end{longtable}

Acceptance criterion: every NP test must return finite numerical output
or a controlled exception.  Silent NaNs, infinities, negative risk values
where positivity is required, and unstable covariance outputs fail the
test plan.

% ─────────────────────────────────────────────────────────────────────────────
\section{Behavioral Confirmation Testing}
\label{sec:behavioral-confirmation}

Behavioral confirmation tests encode model facts that should hold before
any benchmark comparison is meaningful.  These are not external goldens;
they are internal consistency checks based on financial logic.  The
implemented behavioral tests are BEH\_01 through BEH\_08 in
\code{test\_backend.py}.

\subsection{Black-Scholes Behavioural Properties}

\begin{center}
\begin{tabular}{L{2.2cm}L{4.8cm}L{6cm}}
\toprule
\textbf{Test ID} & \textbf{Property} & \textbf{Expected behavior} \\
\midrule
\code{BEH\_01} & Call monotonicity in spot & A European call price should increase as $S$ increases. \\
\code{BEH\_02} & Call monotonicity in strike & A European call price should decrease as $K$ increases. \\
\code{BEH\_03} & Call monotonicity in volatility & A European call price should increase as $\sigma$ increases. \\
\code{BEH\_04} & Put-call parity & The implementation should satisfy
$C - P = S e^{-qT} - K e^{-rT}$ within floating-point tolerance. \\
\code{BEH\_05} & Volatility-to-zero limiting case & As volatility tends to zero, the option value should approach intrinsic / deterministic discounted value. \\
\code{BEH\_06} & No-arbitrage lower bound & The call price should not fall below $\max(S e^{-qT} - K e^{-rT}, 0)$. \\
\bottomrule
\end{tabular}
\end{center}

These tests check that the implementation behaves correctly between
known limiting cases and does not violate elementary no-arbitrage logic.

\subsection{Risk Measure Coherence and Positivity}
...
\textbf{Test ID} & \textbf{Property} & \textbf{Expected behavior} \\
\midrule
\code{BEH\_07} & ES and VaR internal consistency & Historical, parametric, and Monte Carlo methods should satisfy ES $\geq$ VaR when evaluated at the same confidence level. \\
\code{BEH\_08} & Historical VaR positivity & Historical VaR should be finite and positive on the representative two-stock fixture. \\
\bottomrule
\end{tabular}
\end{center}

Acceptance criterion: behavioural facts must hold before a model output is
treated as meaningful.  If these basic implications fail, the issue is
more serious than a calibration error, because it indicates a pricing,
aggregation, or risk-measure implementation defect.

% ─────────────────────────────────────────────────────────────────────────────
\section{Convergence and Inversion Testing}
\label{sec:convergence-inversion}

Convergence and inversion tests address numerical robustness rather than
only formula correctness.  A production-style numerical method should
be stable as discretisation or simulation parameters change, and an
inversion routine should reprice or recover the target quantity when
its output is substituted back into the model.

\subsection{Monte Carlo Convergence}

\begin{itemize}
  \item \textbf{CONV\_01} checks that Monte Carlo VaR becomes more stable
    as the number of simulated paths increases from $500$ to $5{,}000$
    to $50{,}000$.
\end{itemize}
...
\]
The implemented test therefore checks that the fine-grid difference
$|\mathrm{VaR}_{50k} - \mathrm{VaR}_{5k}|$ is smaller than the coarse-grid
difference $|\mathrm{VaR}_{5k} - \mathrm{VaR}_{500}|$ under a fixed random
seed.

\subsection{Merton and Kupiec Inversion Checks}

\begin{center}
\begin{tabular}{L{2.2cm}L{4.8cm}L{6cm}}
\toprule
\textbf{Test ID} & \textbf{Test} & \textbf{Expected behavior} \\
\midrule
\code{INV\_01} & Merton implied-barrier round-trip & Given $V_0$, $B$, $r$, $\sigma$, and $T$, compute the Merton default probability, convert it to target survival, invert for $B$, and recover the original barrier within tolerance. \\
\code{INV\_02} & Kupiec exact-count p-value & For $250$ observations and an exception count close to the expected count under the selected confidence level, the Kupiec test should not reject unconditional coverage. \\
\bottomrule
\end{tabular}
\end{center}

Acceptance criterion: convergence tests should demonstrate improved
stability as simulation count increases, and inversion tests should
recover the original input or fail with a clear diagnostic rather than
silently returning an inconsistent quantity.

% ─────────────────────────────────────────────────────────────────────────────
\section{P\&L Attribution and Hedge Effectiveness}
\label{sec:pnl-hedge-effectiveness}

Robustness testing in model validation includes checking whether modeled
risk factors explain observed or simulated portfolio value changes.  This
project includes a small P\&L attribution test and a one-day hedge
effectiveness test.  These tests are deliberately scoped to the course
risk engine; a full dynamic hedging engine is out of scope.

\subsection{P\&L Attribution}

\begin{itemize}
  \item \textbf{PNL\_01} checks a linear stock-only portfolio where the
    delta-explained P\&L equals the actual P\&L exactly.
\end{itemize}

For a linear portfolio with share quantities $q_i$ and prices $S_{i,t}$,
actual one-period P\&L is
\[
  \Delta V_t
  =
  \sum_i q_i (S_{i,t+1} - S_{i,t}).
\]
Because the position is linear in the underlying prices, the delta-based
...
\begin{itemize}
  \item \textbf{HEDGE\_01} checks that a one-day delta hedge reduces the
    absolute P\&L of an at-the-money call under both $+1\%$ and $-1\%$
    spot shocks.
\end{itemize}

The test uses the Black-Scholes delta at the starting spot, applies a
small spot shock, and compares the absolute unhedged option P\&L with
the absolute delta-hedged P\&L.  The required behavior is:
\[
...
This is not a claim that the project implements production dynamic
hedging.  It only confirms that the local delta calculation has the
expected first-order hedge interpretation over a small one-day shock.
...
Lecture~5 requires that testing assess \emph{stability and robustness},
including sensitivity to market conditions and deal specifications,
behavior over a large range of inputs, and identification of where the
model performs poorly.  This section formalizes those requirements.

\subsection{Parameter Sensitivity}

The following test cases probe sensitivity to the key model-configuration
parameters that are subject to expert judgment.

\begin{itemize}
  \item \textbf{ROB\_01} Lookback window sweep: run historical and parametric
    VaR at 60, 126, 252, and 504 days.  Expected: VaR varies with the
    captured volatility regime; no crashes; no NaN outputs.
  \item \textbf{ROB\_02} EWMA half-life $N$ sweep: run EWMA covariance at
    $N \in \{10, 20, 60, 120\}$.  Expected: covariance PSD at all values;
    estimates vary smoothly; $N=10$ is more responsive, $N=120$ is more stable.
  \item \textbf{ROB\_03} Monte Carlo path-count sweep: VaR at $n \in
    \{100, 1\,000, 10\,000\}$.  Expected: VaR converges as $n$ increases;
    result at $n = 100$ is noisier but finite; at $n = 10\,000$ is stable.
  \item \textbf{ROB\_04} Extreme confidence levels: VaR and ES at
    $\alpha \in \{0.95, 0.975, 0.99, 0.999\}$.  Expected: strictly
    monotonically increasing; ES $\geq$ VaR throughout.
  \item \textbf{ROB\_05} Very short lookback (30 days): expected to return
    a descriptive error or reduced precision result; no crash.
  \item \textbf{ROB\_06} Very long lookback (2,520 days): expected to handle
    large covariance matrices without memory error; result finite.
\end{itemize}

These are implemented in \code{tests/test\_coverage\_gaps.py} and
\code{tests/test\_strict\_numerics.py} (see Appendix CONV\_05 and CONV\_06).

\subsection{Extreme and Boundary Input Tests}

\begin{itemize}
  \item \textbf{ROB\_07} Near-expiry option ($T = 1/252$): BS price at
    intrinsic-value limit; delta near 0 or 1; no NaN.
  \item \textbf{ROB\_08} Deep OTM put (strike = 0.5 $\times$ spot): small
    positive price; delta near zero; VaR dominated by underlying.
  \item \textbf{ROB\_09} High-volatility input ($\sigma = 2.0$, i.e.\ 200\%):
    VaR large but finite; no overflow.
  \item \textbf{ROB\_10} Zero-drift portfolio ($\mu = 0$): parametric VaR
    still positive from variance term; historical and MC also positive.
  \item \textbf{ROB\_11} Single-position portfolio: all three risk engines
    return positive, finite VaR; no failure from degenerate covariance.
  \item \textbf{ROB\_12} Large portfolio (50 positions): covariance assembly,
    risk computation, and service orchestration complete without crash.
\end{itemize}

\subsection{Known Conditions Where the Model Performs Poorly}

A sound validation program explicitly documents where the model is
expected to degrade, not just where it succeeds.  These cases are
\emph{documented limitations}, not unresolved bugs.

\begin{itemize}
  \item \textbf{Historical simulation.}  The equally-weighted method is
    slow to adapt to volatility regime changes.  This manifests directly
    in the backtesting results: the Christoffersen independence test
    rejects the independence hypothesis at $p < 10^{-14}$, indicating
    clustered exceptions during high-volatility sub-periods.  Filtered
    Historical Simulation or GARCH-based forecasting is the documented
    improvement path.
  \item \textbf{Delta-normal parametric.}  Treats the portfolio as linear
    in underlying returns.  For portfolios with significant gamma (e.g.,
    short options), it systematically underestimates tail losses.  Test
    BEH\_01 through BEH\_05 confirm directional correctness; behavioral
    comparison tests confirm that options-heavy portfolios show materially
    higher historical and MC VaR relative to parametric.
  \item \textbf{Fat-tailed return distributions.}  When actual returns
    exhibit heavy tails (crisis periods, individual-stock events), the
    parametric normality assumption underestimates extreme losses.  The
    exact GBM lognormal module provides a tractable analytical benchmark;
    EVT tail fitting is documented as a future enhancement.
  \item \textbf{Model cascading into capital calculations.}  The VaR
    output feeds \code{src/risk/regulatory.py} capital requirements.
    ROB tests confirm that extreme VaR inputs (very large, near-zero, and
    zero) do not produce silent failures in downstream RWA and ratio
    calculations.
  \item \textbf{Illiquid or discontinuous markets.}  Option repricing
    assumes continuously tradeable underlying without gaps.  Stale prices
    or trading halts in the historical window will affect all three
    methods; the data validation layer flags stale-price runs but does
    not adjust the risk estimate.
\end{itemize}
...
The primary behavioural confirmation tests are now documented in
Section~\ref{sec:behavioral-confirmation}.  The implemented BEH\_01
through BEH\_08 cases cover Black-Scholes monotonicity, put-call parity,
volatility-to-zero behavior, no-arbitrage lower bounds, ES/VaR internal
consistency, and finite positive historical VaR.

\subsection{Convergence and Stability Tests}

The primary convergence and inversion tests are now documented in
Section~\ref{sec:convergence-inversion}.  Additional robustness and
sensitivity tests remain relevant:

\begin{itemize}
  \item \textbf{CONV\_01} MC VaR convergence as simulation count increases
  \item \textbf{CONV\_02} One-asset MC ES convergence toward exact lognormal ES as $n_{\mathrm{sims}}$ increases
...
\begin{itemize}
  ...
  \item \code{tests/test\_numerical\_precision.py}
...
\bibitem[Gol91]{Gol91}
Goldberg, D. (1991).
\textit{What Every Computer Scientist Should Know About Floating-Point Arithmetic}.
